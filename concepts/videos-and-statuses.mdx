---
title: "Videos and statuses"
description: "The lifecycle of a video and the shape every endpoint returns."
---
...
There is no `ready`, `failed` or `done` status on a video. (`done` and `failed` exist on [jobs](/guides/async-jobs).)
