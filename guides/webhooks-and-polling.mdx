...
description: "The exact payload of every event, how to verify it, and the reconciler you should run beside it."
---
...
<Tabs>
  <Tab title="video.rendered / video.render_failed">
    ...
    On `video.render_failed`, `status` is `error`, `error` has the reason, `renderUrl` is null, and the render charge has been refunded. `externalUserId` is set for videos made in an [embedded studio](/guides/embedded-studio) session.
  </Tab>
