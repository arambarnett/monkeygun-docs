...
description: "An episode every day, or every time your data moves, with no scheduler on your side."
---
...
Threshold rules (a rate crosses X, TVL moves Y percent, a wallet moves more than Z) are not in `start_series` today. Run those on your side and call `from-data`, or ask us to run them as part of a [Data Shows](/packages) engagement.
